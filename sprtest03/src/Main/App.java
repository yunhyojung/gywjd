...
public class App { //자바로 회원관리하는 방법 알아보기(실제로 사용하지는 않음)-dependencies
	private static Assembler assembler = new Assembler(); //Assembler에 있는 의존객체 사용하여 객체 주입
...
//			MemberRegisterService mrs = new MemberRegisterService(memberDAO); //의존객체(dependency object)
			MemberRegisterService mrs = assembler.getMemberRegisterService();//assembler사용하여 불러오기만 하면 됨
			mrs.regist(req);
			   ...
			   ChangePasswordService changePwSvc = assembler.getChangePasswordService();
