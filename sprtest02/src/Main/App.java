...
public class App { //자바로 회원관리하는 방법 알아보기(실제로 사용하지는 않음)-dependencies
	private static MemberDAO memberDAO = new MemberDAO();//추가(의존객체 사용 위해)
	//위의 MemberDAO를 바꿀 경우 private static MemberDAO memberDAO = new CacheMemberDAO(); 이렇게 바꿔 사용할 수 있음(유지 보수 편리)
	private static MemberPrinter printer = new MemberPrinter();
     ...
     public static void main(String[] args) { //main메소드 : ***컨트롤러 부분이라 생각하면 됨***
		...
		if(command.startsWith("new")) { //포함되는 글자로 찾기(startsWith로 스플릿을 하지 않아도 됨)
			...
			MemberListPrinter listPrint = new MemberListPrinter(memberDAO,printer); //의존객체 만듦
			listPrint.printAll();
		   ...
		   if(arg.length != 2) {
			...
			infoPrinter.setMemberDAO(memberDAO); //의존객체 주입
